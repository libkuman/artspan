...
";
    ...
    $select[16] = '
                   GROUP_CONCAT(DISTINCT CONCAT(website_civireport.url, 
                                       CASE
                                         ...
                                         WHEN website_type_id = 3 THEN " (Facebook)"
                                         WHEN website_type_id = 4 THEN " (Twitter)"
                                         WHEN website_type_id = 5 THEN " (MySpace)"
                                         WHEN website_type_id = 6 THEN " (Main)"
                                         WHEN website_type_id = 8 THEN " (Flickr)"
                                         WHEN website_type_id = 1 THEN " (Home)" 
                                       END)
  ...
  function from() {
    ...
    $this->_from .= "
             ...
             LEFT JOIN  civicrm_phone public_phone
                    ON ({$this->_aliases['civicrm_contact']}.id = public_phone.contact_id AND
                       public_phone.location_type_id = 7)
      ";
    //END OVERRIDE BY MARK LIBKUMAN OPENFLOWS ARTSPAN 5/23/2014
    /////////////////////////////////////////////////////////////////
