...
class CRM_Event_Form_Registration_Register extends CRM_Event_Form_Registration
{
    ...
    {  
        ...
        $this->assign( 'hasAdditionalParticipants', $hasAdditionalParticipants );
         ...
         //$this->_defaults['credit_card_exp_date'] = array( 'Y' => '2013', 'M' => '05' );
        ...
        //get weekend taxonomy terms
        $we_terms = taxonomy_get_tree(5);
        $weekends[] = 'Select Weekend Location';
        foreach($we_terms as $weekend) {
            if (ereg('Weekend', $weekend->name))
                $weekends[$weekend->tid] = $weekend->name;
            else
                $weekends[$weekend->tid] = '-- ' . $weekend->name;
        }
        $this->assign('weekends', $weekends);
        
        // Prepare other data for smarty
        ...
        $this->assign('studios', $group_studios);
        //$this->addRule( 'studio_group_location', ts('Group Location Reference is a required field.'), 'required' );
	$this->_validStudioList = $group_studios;
        ...
        if($contactID) {
          ...
          $contact = _artspan_get_civi_contact_public_location_info(FALSE, array('contact_id' => $contactID, 'return_custom_45' => 1, 'return_custom_3' => 1, 'return_custom_4' => 1));

          $this->_artspan_info = $contact;
          $this->assign('studio_title', $contact['custom_45']);
          
          $group_studio = isset($contact['address']['shared']) && $contact['address']['shared']->contact_sub_type == 'Group_Studio_Site';
          $this->assign('group_site_yes', $group_studio);

          if($group_studio) {
            $this->assign('studio_selected', $contact['address']['shared']->id);
            $this->assign('studio_bldg_no', $contact['custom_3']);
            $this->assign('studio_no', $contact['custom_4']);
            $this->assign('state_selected',array_search('California', $states));
          }
          else {
            $this->assign('studio_selected', 0);
            $this->assign('studio_address', $contact['address']['street_address']);
            $this->assign('studio_city', $contact['address']['city']);
            $this->assign('state_selected', $contact['address']['state_province_id']);
            $this->assign('studio_zip', $contact['address']['postal_code']);
          }
        }
    ...
    public function postProcess() 
    {
       ...
       // Update group studio information
       // note: it's kind of lame that this is happening here, but due to time constraints
       // the hacky solution will have to do. at least the information is getting saved...
       ...
       $contact_params = array(
        'contact_id' => $this->_artspan_info['contact_id'],
