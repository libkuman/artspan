...
class CRM_Event_Form_Registration_Confirm extends CRM_Event_Form_Registration
{
    ...
    { 
        ...
        if ($params[0]['studio_weekend_no']) {
          $term = taxonomy_get_term($params[0]['studio_weekend_no']);
          $parents = taxonomy_get_parents($params[0]['studio_weekend_no']);
          foreach($parents as $getparent) {
            $parent = $getparent;
          }
          $weekend1_text = $parent->name . ': ' . $term->name;
          $this->assign( 'weekend1_text', $weekend1_text );
        }

        if ($params[0]['studio_weekend_no_2']) {
          $term = taxonomy_get_term($params[0]['studio_weekend_no_2']);
          $parents = taxonomy_get_parents($params[0]['studio_weekend_no_2']);
          foreach($parents as $getparent) {
            $parent = $getparent;
          }
          $weekend2_text = $parent->name . ': ' . $term->name;
          $this->assign( 'weekend2_text', $weekend2_text );
        }
    }
    ...
    public function postProcess( ) 
    {
        ...
        if ( ( $this->_contributeMode == 'checkout' ||
            ...
            // Artspan stuff -- give drupal user profile the correct taxonomy term
            _artspan_load_civi_api();
            $user = civicrm_uf_id_get($contactId);
            if($user) {
              _artspan_openstudios_register($user, $this->_values['event'], $this->_values['params']);
            }
        }
    } //end of function
